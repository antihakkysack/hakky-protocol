...
- [ ] **Step 4: Implement the narrow decoder**
  ...
  | pinned launchpad IDL `migrate_to_amm` | discriminator `[207,82,192,145,254,207,145,223]`; exact ordered 32 accounts as normalized in the AMM-v4 return contract; args are `u64LE base_lot_size`, `u64LE quote_lot_size`, `u8 market_vault_signer_nonce`; fixed LaunchLab, AMM-v4, classic Token, ATA, System, and Rent programs must match pinned identities |
  | pinned launchpad IDL `create_platform_config` / `update_platform_config` | create discriminator `[176,90,196,175,253,113,220,20]`; update discriminator `[195,60,76,129,146,45,67,143]`; exact account order and signer flags from the return contracts; PDA seeds are UTF-8 `platform_config` plus `platform_admin`; update variants and every `AllInfo` field define the exhaustive mutable-field set |
...
- [ ] **Step 4: Implement finalized RPC collection**
  ...
  Fetch the finalized creation transaction first and derive `minContextSlot` from its slot. Change all mint, creator-token-account, metadata, and block-time reads to `finalized`. Preserve response context slots, hash exact account bytes before decoding, and reject mixed observations whose slot or block time cannot be established. Fetch the content-addressed metadata JSON and image named by the on-chain account and require byte equality with the validated manifest/readback pair. Validate `creatorPayment.debitLamports` but do not serialize it in mint-v2, whose exact schema has no `cost`; LaunchLab-v2 Task 4 consumes the paired readback and records the debit in its common cost object. Never serialize the RPC URL; retain only the public hostname after credential/query validation.
...
- [ ] **Step 4: Implement transaction/account collection**
  ...
  Validate the exact Operations Task 2 manifest/readback pair first. Fetch the finalized creation transaction as raw/versioned transaction data and resolve address lookup tables before decoding. Separately fetch launch, base vault, quote vault, metadata, and PlatformConfig bytes using `minContextSlot` equal to the finalized transaction slot. Because InitializeV2 contains the PlatformConfig address but not its administrator, trace that account's finalized creation history, decode the exact `create_platform_config` instruction, require its signer and derived PDA to match, and bind that administrator plus every pinned `update_platform_config` variant/`AllInfo` field into the exhaustive `updateAuthorities`/`mutableFields` evidence. If complete finalized history is unavailable, hard stop; never infer the administrator from an API or label. Treat Raydium API data as optional diagnostics only and exclude it from required checks.
...
- [ ] **Step 6: Implement the append-only CLI**
  ...
  Parse only public identifiers, `--metadata-manifest`, `--metadata-readback`, repeated public `--recovery-transaction <signature>` values, and an HTTPS RPC URL. Fetch the content-addressed metadata bytes independently, reject manifest/readback/digest or creator-payment drift, derive every debit from finalized chain data, reject alternate output paths, and publish exactly `proof/mainnet-launchlab.json`. Add package script:
