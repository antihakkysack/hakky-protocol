...
- Token identity is exactly `Hakky Protocol` / `HAKKY`, supply `10,000,000`, decimals `6`, and base-unit supply `10000000000000`.
- Allocation is `8000000000000` base units on the public curve, `2000000000000` for post-graduation liquidity, zero vesting, zero creator allocation, and no creator first-buy.
...
Before any immutable-economic approval check, `evaluateLaunchPreview()` must call `evaluateHakkyLaunchlabSourceCoverage()` with only decoded source values. At the approved docs pin the exact HAKKY target returns `source-coverage-verified`, then the independent `platform-config-immutability-unavailable` control must fail because the binding PlatformConfig remains admin-mutable until migration. The combined evaluation cannot return `ok: true`, and `buildApprovalEnvelope()` cannot emit an envelope. A current PlatformConfig value, UI toggle, screenshot, operator assertion, deleted ephemeral key, or injected coverage object is not a substitute for on-chain immutability.
